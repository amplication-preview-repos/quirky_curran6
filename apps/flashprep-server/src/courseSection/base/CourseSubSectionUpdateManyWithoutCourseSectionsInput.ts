/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { CourseSubSectionWhereUniqueInput } from "../../courseSubSection/base/CourseSubSectionWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class CourseSubSectionUpdateManyWithoutCourseSectionsInput {
  @Field(() => [CourseSubSectionWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [CourseSubSectionWhereUniqueInput],
  })
  connect?: Array<CourseSubSectionWhereUniqueInput>;

  @Field(() => [CourseSubSectionWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [CourseSubSectionWhereUniqueInput],
  })
  disconnect?: Array<CourseSubSectionWhereUniqueInput>;

  @Field(() => [CourseSubSectionWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [CourseSubSectionWhereUniqueInput],
  })
  set?: Array<CourseSubSectionWhereUniqueInput>;
}

export { CourseSubSectionUpdateManyWithoutCourseSectionsInput as CourseSubSectionUpdateManyWithoutCourseSectionsInput };
